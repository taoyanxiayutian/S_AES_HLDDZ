...
public class Decrypt {
    ...
    public static String decryptASCII(String ciphertextASCII, String key) {
        int[][] ciphertextArray = stringToIntArray(ciphertextASCII);
        ...
        return intArrayToString(plaintextArray);
    }
    ...
    public static int[] decryptData(int[] ciphertext, String key) {
        ...
        int[] block = permute(IP, ciphertext);  // 初始置换
        block = roundFunction(k2, block);  // 第一轮函数
        block = swapHalves(block);  // 交换左右半部分
        block = roundFunction(k1, block);  // 第二轮函数
